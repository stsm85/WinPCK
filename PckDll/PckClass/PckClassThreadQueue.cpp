...
BOOL CPckClassThreadWorker::getCompressedDataQueue(LPBYTE &lpBuffer, PCKINDEXTABLE_COMPRESS &lpPckIndexTable)
{
	...
	if(0 != lpPckIndexTable.dwIndexDataLength) {
		if(1 == (int)lpBuffer) {
			logOutput(__FUNCTION__, "id(%05d), lpBuffer(0x%08x):0x0000000000000000, szFile(%s), esize:%d, dsize:%d\r\n", 
			...
			logOutput(__FUNCTION__, "id(%05d), lpBuffer(0x%08x):0x%016llx, szFile(%s), esize:%d, dsize:%d\r\n", 
				mt_dwCurrentQueuePosGet,
				(int)lpBuffer, 
...
void CPckClassThreadWorker::uninitCompressedDataQueue()
{
	m_QueueContent.clear();
	m_Index_Compress.clear();
	SetThreadFlag(FALSE);
